...
class GameBoard:
	...
	def __init__(self):
		for x in range(8):
			...
			for y in range(8):
				if (y == 0):
					self._pieceMap[x].append(Pawn("white", False, Direction.UP))
				elif(y == 7):
				  self._pieceMap[x].append(Pawn("black", False, Direction.DOWN)) 
				else:
	...
	def _isCheckmate(self):
		...
		for x in range(len(self.pieceMap)):
			for y in range(len(self.pieceMap[x])):
				piece = self.pieceMap[x][y]
				if piece != None and piece.isKing == True:
					kingPosList.append(BoardPosition(x, y))

		isCheckmate = False
		for kingPos in kingPosList:
			if (self._isThreatenend(kingPos) == True):
				isCheckmate = True
		return isCheckmate

	def _isThreatenend(self, pos):
		isThreatenend = False
		for x in len(self.pieceMap):
			for y in len(self.pieceMap[x]):
				piece = self.pieceMap[x][y]
	...
	def getAllPossibleMovePosOfPiece(self, piecePos):
	  ...
	  for movePos in movePosList:
	    if self._isMoveCheckmate(piecePos, movePos) == False:
	...
	def changePiece(self, pos, toPiece):
		print("")

	def getPiece(self, boardPosition):
		return self.pieceMap[boardPosition.X][boardPosition.Y]
